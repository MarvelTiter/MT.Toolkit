...
namespace MT.Toolkit.LogTool
{
    ...
    public static class LogInfoExtensions
    {
        public static string FormatLogMessage(this LogInfo self)
        {
            ...
            return $"{self.LogHeader()}{self.LogCategory()}{self.LogScope()}{self.LogBody()}";
        }
        public static string LogScope(this LogInfo self)
        {
            if (self.Scopes?.Count > 0)
            {
                return $"    {string.Join(" => ", self.Scopes)}{Environment.NewLine}";
            }
            return string.Empty;
        }
